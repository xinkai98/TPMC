...
def _load_places(batch_size=128, image_size=84, num_workers=8, use_val=False):
    ...
    data_dir = './places365standard_easyformat'
    if os.path.exists(data_dir):
        fp = os.path.join(data_dir, 'places365_standard', partition)
        if not os.path.exists(fp):
            print(f'Warning: path {fp} does not exist, falling back to {data_dir}')
...
class CutoutColor(object):
    ...
    def __init__(self, batch_size, box_min=10, box_max=30, pivot_h=12, pivot_w=24, obs_dtype='uint8', *_args,
                 **_kwargs):
    ...
    def do_augmentation(self, imgs):
        ...
        for i, (img, w11, h11, pivot_w, pivot_h) in enumerate(zip(imgs, w1, h1, pivot_w_, pivot_h_)):
            ...
            for j in range(box_num):
                cut_img[:, pivot_h[j] + h11[j]:pivot_h[j] + h11[j] + h11[j],
                pivot_w[j] + w11[j]:pivot_w[j] + w11[j] + w11[j]] = np.tile(
                    rand_box[i][j].reshape(-1, 1, 1),
                    (1,) + cut_img[:, pivot_h[j] + h11[j]:pivot_h[j] + h11[j] + h11[j],
                           pivot_w[j] + w11[j]:pivot_w[j] + w11[j] + w11[j]].shape[1:])
