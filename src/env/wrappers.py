...
import dmc2gym  # 这里使用的 dmc2gym 为 DMControl-GB 环境（来自于 SODA）
...
def make_env(domain_name,
    ...
    if not is_distracting_cs:
        env = VideoWrapper(env, mode, seed)  # 如果 mode 中有 video 则 返回的观测应用 greenscreen， 否则原观测
    env = FrameStack(env, frame_stack)  # 多帧堆叠
    if not is_distracting_cs:
        env = ColorWrapper(env, mode, seed)  # 返回正常的观测
...
class VideoWrapper(gym.Wrapper):
    ...
    # print("video path-", self._get_video_paths)
    # print("number of videos in the path", self._num_videos)

    def _get_video_paths(self):
        video_dir = os.path.join('env/data', self._mode)  # modified
        print("inside get video paths-", video_dir)
        if 'video_easy' in self._mode:
            self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(10)]
            print("video_paths", self._video_paths)
    ...
    def _load_video(self, video):
        ...
        n = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))  # 帧数
    ...
    def _reset_video(self):
        # self._index = (self._index + 1) % self._num_videos  # 用下一张视频

        print("current video:", self._video_paths[self._index])
